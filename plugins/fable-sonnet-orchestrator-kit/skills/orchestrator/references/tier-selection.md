...
costing *more* than a dearer one. This reference is the rubric of record.
The enforceable rules live in the parent `orchestrator/SKILL.md`, and each
tier maps to a model-pinned Codex custom agent under the source repo's
...
on the issue *is* the model that does the work: Haiku → GPT-5.6-Luna/max,
Sonnet → GPT-5.6-Terra/max, and Opus → GPT-5.6-Sol/max.
...
1. **Escalate on bounce; do not haiku-retry.** A `tier:haiku` PR that draws
   ...
   the model is pinned in the custom-agent TOML, so "escalate" means a fresh
   dispatch at the higher tier — **not** a follow-up to the
...
- **`pr-reviewer` stays pinned GPT-5.6-Terra/max even for `tier:haiku` PRs.**
